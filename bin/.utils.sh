...
function _start_worker
{
	...
	cgexec -g cpuset:$LOTUS_CUSTOM_CPUGROUP --sticky lotus-worker run --listen=0.0.0.0:$LOTUS_CUSTOM_PORT --addpiece=$LOTUS_CUSTOM_ADDPIECE --precommit1=$LOTUS_CUSTOM_PRECOMMIT1 --precommit2=$LOTUS_CUSTOM_PRECOMMIT2 --commit=$LOTUS_CUSTOM_COMMIT --unseal=$LOTUS_CUSTOM_UNSEAL --replica-update=$LOTUS_CUSTOM_REPLICA_UPDATE --prove-replica-update2=$LOTUS_CUSTOM_PROVE_REPLICA_UPDATE2 --regen-sector-key=$LOTUS_CUSTOM_REGEN_SECTOR_KEY
    fi
}
