...
do
  ...
  read -p "Replica update? [true/false] "         -i $LOTUS_CUSTOM_REPLICA_UPDATE -e LOTUS_CUSTOM_REPLICA_UPDATE
  read -p "Prove replica update 2? [true/false] " -i $LOTUS_CUSTOM_PROVE_REPLICA_UPDATE2 -e LOTUS_CUSTOM_PROVE_REPLICA_UPDATE2
  read -p "Regen sector key? [true/false] "       -i $LOTUS_CUSTOM_REGEN_SECTOR_KEY -e LOTUS_CUSTOM_REGEN_SECTOR_KEY
